...
from share_model import Model, Net_out_loss
...
if __name__ == '__main__':
    ...
    n_tasks = 6
    epochs = 30000
    learningrate = 0.0021062372212595124
    MinTrainLoss = 1e10
    R2_pre = -1
    test_pre = [-1]
    train_pre = [-1]
    ...
    Net = Model(num_feature=10, num_experts=4, units=8, expert_hidden=[10, 9, 6],
                hidden_units=[7, 12], n_tasks=n_tasks).to(device=device)
    optimizer = torch.optim.Adam(Net.parameters(), lr=learningrate)
    loss_fun = nn.MSELoss()

    model_ = 'E://Machine learning/Database/H2/model_finish_new/'
    model_name = model_ + 'share_' + feature_ + '_MOF_base_ALL.pth'

    aoc_train, aoc_test, mse_test = TrainModel(Net,
                                        x_train, y_train,
                                        n_tasks, loss_fun, optimizer, epochs,
                                        x_test=x_test, y_test=y_test,
                                        save=True, save_name=model_name).train()
